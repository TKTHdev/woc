...
func RecordBatchTS(latencyMs float64, isErr bool) {
	if isErr {
		return
	}
	tsOpsTotal.Add(1)
	tsLatSumMs.Add(int64(latencyMs * 100))
	tsLatCount.Add(1)
}
...
func startTimeSeriesRecorder(clientID int, intervalMs int) {
	...
	if _, err := fmt.Fprintln(f, "elapsed_ms,tps,lat_avg_ms,event"); err != nil {
		_ = f.Close()
	...
	go func() {
		...
		for range ticker.C {
			...
			if _, err := fmt.Fprintf(f, "%d,%.1f,%.2f,%s\n", elapsed, tps, latAvg, event); err != nil {
		...
		if pipelined {
			...
			go func(clockVal int, connection *rpc.Client, command *Args, serverID int, batchSize int) {
				...
				if err != nil {
					...
					RecordBatchTS(reply.Latency, false)
