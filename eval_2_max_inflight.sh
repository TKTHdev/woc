...
start_workload_nodes() {
    ...
    for i in "${!SERVER_IPS[@]}"; do
        ip="${SERVER_IPS[$i]}"
        remote_exec "$ip" "pkill -x woc 2>/dev/null || true; cd '$REMOTE_DIR'; nohup '$REMOTE_DIR/$BINARY' -id=$i -path='$CONFIG_PATH' -et=1 -n=$NUM_SERVERS -t=$THRESHOLD -b=$BATCHSIZE -mode=1 -mcli=$MONGO_CLIENT_POOL -mload='$WORKLOAD' -bcomp=object-specific -indep=$INDEP_RATIO -common=$COMMON_RATIO -pipeline=$PIPELINE_MODE -log=$LOG_LEVEL -ep=true -role=0 > '$LOG_DIR/server_${i}_inflight_${max_inflight}.log' 2>&1 &"
    done
    ...
    for i in "${!CLIENT_HOST_IPS[@]}"; do
        ...
        remote_exec "$ip" "pkill -x woc 2>/dev/null || true; MAX_INFLIGHT=$max_inflight cd '$REMOTE_DIR'; nohup '$REMOTE_DIR/$BINARY' -id=$client_id -path='$CONFIG_PATH' -et=1 -n=$NUM_SERVERS -t=$THRESHOLD -b=$BATCHSIZE -mode=1 -mload='$WORKLOAD' -bcomp=object-specific -indep=$INDEP_RATIO -common=$COMMON_RATIO -pipeline=$PIPELINE_MODE -log=$LOG_LEVEL -ops=0 -role=1 > '$LOG_DIR/client_${i}_inflight_${max_inflight}.log' 2>&1 &"
    done
}
