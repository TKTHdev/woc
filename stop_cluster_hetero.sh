#!/bin/bash
# ================================================================
# WOC Heterogeneous Cluster Stopper (Clients → Servers)
# Mirrors stop_cluster.sh but targets the cora hetero topology used by
# start_mongodb_hetero.sh. Gracefully stops woc, then collects + merges
# eval CSVs from every node.
# ================================================================
...
# Must match SERVER_IPS / CLIENT_HOST_IPS in start_mongodb_hetero.sh
ALL_SERVER_IPS=(
"192.168.73.93"   # cora-c32-1
"192.168.73.107"  # cora-c32-2
"192.168.73.79"   # cora-c8-1
"192.168.73.183"  # cora-c8-2
"192.168.73.211"  # cora-c8-3
)
...
# Optional overrides when experiment sizes change:
#   SERVER_COUNT=3 CLIENT_COUNT=1 ./stop_cluster_hetero.sh
SERVER_COUNT="${SERVER_COUNT:-${#ALL_SERVER_IPS[@]}}"
CLIENT_COUNT="${CLIENT_COUNT:-${#ALL_CLIENT_IPS[@]}}"
...
if [ "$CLIENT_COUNT" -lt 0 ] || [ "$CLIENT_COUNT" -gt "${#ALL_CLIENT_IPS[@]}" ]; then
    echo "ERROR: CLIENT_COUNT must be between 0 and ${#ALL_CLIENT_IPS[@]}"
    exit 1
fi

SERVER_IPS=("${ALL_SERVER_IPS[@]:0:$SERVER_COUNT}")
CLIENT_IPS=("${ALL_CLIENT_IPS[@]:0:$CLIENT_COUNT}")

SERVER_ID_FILTER="0-$((${#SERVER_IPS[@]} - 1))"
CLIENT_START_ID="${#SERVER_IPS[@]}"
CLIENT_END_ID="$((CLIENT_START_ID + ${#CLIENT_IPS[@]} - 1))"
CLIENT_ID_FILTER=""
if [ "${#CLIENT_IPS[@]}" -gt 0 ]; then
...
echo "=================================================="
echo " WOC Hetero Cluster Shutdown (Clients → Servers)"
echo "=================================================="
...
for i in "${!CLIENT_IPS[@]}"; do
    ...
    copy_eval_dir "${CLIENT_IPS[$i]}" "client${client_id}" "client${client_id}" || true
done
...
if [ -f "${MERGE_SCRIPT}" ]; then
    if [ "${#CLIENT_IPS[@]}" -eq 0 ]; then
        echo " - No clients configured; skipping client merge"
    else
